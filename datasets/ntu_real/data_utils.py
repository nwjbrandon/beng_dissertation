...
def init_pose3d_labels(param_file_path, pose3d_gt_path):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        cropped_cam_param = (
            np.array([cam_param[0], cam_param[1], cam_param[2] - bbox[0], cam_param[3] - bbox[1]])
            * resize_ratio
        )
...
def visualize_data(im_path, local_pose3d_gt, local_mesh_pts_gt, cam_param, mesh_tri_idx):
    ...
    cam_proj_mat = np.array(
        [[cam_param[0], 0.0, cam_param[2]], [0.0, cam_param[1], cam_param[3]], [0.0, 0.0, 1.0]]
    )
