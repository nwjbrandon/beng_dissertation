...
sys.path.insert(0, "/home/nwjbrandon/fyp/mycobot_teleop/models/yolov5")
    ...
    def detect(self, path, im, im0s, vid_cap, s):
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                detected_hands.append(None)
            im0 = annotator.result()
            output_imgs.append(im0)

        return output_imgs, detected_hands


if __name__ == "__main__":
    weights = ["./hand_object_detection.pt"]
    source = "/dev/video0"
    data = "yolov5/data/hand.yaml"
    conf_thres = 0.25
    iou_thres = 0.45
    max_det = 1000
    device = "cpu"
    ...
    dataset = LoadStreams(source, img_size=detector.imgsz, stride=detector.stride, auto=detector.pt)
    with torch.no_grad():
        for path, im, im0s, vid_cap, s in dataset:
            im0, _ = detector.detect(path, im, im0s, vid_cap, s)
            # Stream results
            cv2.imshow("demo", im0[0])
