...
class NLBlockND(nn.Module):
    def __init__(
        self, in_channels, inter_channels=None, mode="embedded", dimension=3, bn_layer=True
    ):
        ...
        if bn_layer:
            self.W_z = nn.Sequential(
                conv_nd(
                    in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1
                ),
                bn(self.in_channels),
            )
